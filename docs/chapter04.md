...
- 그냥 간단히 생각하여 *policy* \\(\pi\\)를 어떤 특정한 함수, 또는 하나의 자료구조 덩어리라고 생각하자.
    ...
    - 최종적으로 스텝 \\(t\\)마다 \\([state, action]\\)형태의 *Map* 집합으로 이루어진 자료구조를 상상할 수 있다.
        - \\( \pi = \{ \\{ map[s,a]_0, map[s,a]_1, ..., map[s,a]_t\} \\} \\)
...
$$\pi_{sample}^{t} = \{ \pi_0(V)=a, \pi_1(W)=c, \pi_1(Z)=d, \pi_2(P)=a, \pi_2(G)=a \}_{t=0}$$

- 즉, 특정 스텝 \\(t=0\\) 에서 사용될 액션이 무조건 정해져있고 이에 따른 경로가 선택되어 진다.
    - 위의 그림에서 표를 포면 상태 \\(V\\)에서의 액션 정보가 정의되어 있다.
    - \\(t=0\\) 인 상황하에 상태 \\(V\\)에서 사용 가능한 모든 액션 집합은 \\(\\{a, b\\}\\)이다.
    - 하지만 주어진 *policy* \\(\pi_{sample}\\)에서는 \\(t=0\\) 스텝이고 상태 \\(V\\)인 경우 액션 \\(\{ a\}\\)를 선택하도록 기술되어 있다.
- 그리고 선택된 액션에 따라 전이될 수 있는 상태와 이 때의 전이 확률이 모두 달라지게 된다.
- 사실 모든 \\(\pi\\) 에는 \\(t=0\\)에 대해 모든 상태에 대해 사용될 액션이 정의되어 있어야 하는데, 위 샘플에서는 확률 값이 0인 경우는 모두 생략되어 있다.
    - 이게 무슨 말이고 하면 \\t=0 인 상황에서 절대 도달할 수 없는 상태에 대해서는 굳이 기술을 안해도 문제가 없다는 이야기이다.
